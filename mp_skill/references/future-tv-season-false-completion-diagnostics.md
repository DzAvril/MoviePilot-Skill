...
4. Check download and transfer history across enough pages to avoid only seeing recent items:
   ```bash
...
- If MoviePilot cannot hydrate a valid future season via the normal API, use `references/tv-season-postgresql-fallback.md` only after duplicate checks and read-back verification.
...
- **下一步**: recreate/fix subscription, or wait for resources if no exact S02 resources exist.
